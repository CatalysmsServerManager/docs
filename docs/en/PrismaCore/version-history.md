...
* added consolecommand decodesandboxoptions (dsbo): list all sandbox options in human readable format without needing to paste the sandbox code into the client for reviewing options. Lists options both as human readable as technical representation (incl. default values)
Ex: `RangedDamage: 150% (1.5 | default:1)
DropOnQuit: None (0 | default:0)
BackpackCrafting: Yes (1 | default:1)
WorkstationCrafting: Yes (True | default:True)`
...
* Added consolecommand IsBloodMoon: for use with ServerManagers to determine if a Bloodmoon is active at the time of executing the command
* Added consolecommand GetParty: for use in console or ServerManagers to get all party members of a player
* Added consolecommand GetAllies: for use in console or ServerManagers to get all allies of a player

Nigration notes for CPM to PrismaCore:
* /Config/buffs.xml has to be manually updated with any changes you made to the old buffs.xml
* All other files have an easy migration path. The following files can be renamed for migration purposes (rename the CPM variant after deleting the autogenerated PrismaCore files)
  
CpmSetting.xml -> PrismaCoreSettings.xml (rename CpmPrefix to PrismaCorePrefix)

CpmStrings.xml -> PrismaCoreStrings.xml

Permadeath.xml -> PrismaCorePermaDeath.xml

DonorSlots.xml -> PrismaCoreDonorSlots.xml

cpmcc_permissions.xml -> ClaimCreator_permissions.xml

cpmPlayers -> PrismaCorePlayers.db

cpmWaypoints.db -> PrismaCoreWaypoints.db

cpmGroupColors.db -> PrismaCoreGroupColors.db

cpmClaims.db -> PrismaCoreClaims.db

cpmTelespawns.db -> PrismaCoreTelespawns.db

cpmVehicleOwners -> PrismaCoreVehicleOwners.db

**Version 26.0 2.4 b6 Stable (NOT 2.2 b3 Stable compatible) (15-08-2025)**
