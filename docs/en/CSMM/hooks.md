...
::: warning 
the _italics_ hooks require at least Version 9.5 or higher of the [CSMM Patrons Mod (CPM)](https://confluence.catalysm.net/pages/viewpage.action?pageId=1114182) plugin to be installed in order for these event triggers to work.
:::
...
This is where the true creativity of custom hooks can be found. Referred to as _logLine_ Events, these hooks will fire whenever totally unique custom criteria are matched, and this capability is extremely powerful when paired up with [CSMM Patrons Mod (CPM)](https://confluence.catalysm.net/pages/viewpage.action?pageId=1114182)'s Write2Log (W2L) as well. Using W2L, you can create a server event log at any time you wish, allowing you to create a record in the event log that some specific action or event has taken place.
